...
PUBLIC int websOpen(char *documents, char *routeFile)
{
    ...
    if ((accessFd = open(accessLog, O_CREAT | O_TRUNC | O_APPEND | O_WRONLY, 0666)) < 0) {
        ...
        LOGD("Cannot open access log %s", accessLog);
        return -1;
    }
...
PUBLIC int websListen(char *endpoint)
{
    ...
    if ((sid = socketListen(ip, port, websAccept, 0)) < 0) {
        ...
        LOGD("Unable to open socket on port %s:%d. ", ip, port);
        return -1;
    }
    sp = socketPtr(sid);
    sp->secure = secure;
...
static int setLocalHost()
{
    ...
    if (gethostname(host, sizeof(host)) < 0) {
        ...
        LOGD("%s(%d)\n", __FUNCTION__, __LINE__);
        LOGD("Cannot get hostname: errno %d", errno);
        return -1;
    }
    LOGD("%s(%d)gethostname()=%s\n", __FUNCTION__, __LINE__, host);
#if VXWORKS
...
#elif TIDSP
{
  ...
  //  ipaddr = inet_ntoa(intaddr);
	...
	strncpy(ifr.ifr_name, "eth0", IFNAMSIZ);    // Ö¸¶¨Íø¿¨  
...
PUBLIC int websServer(char *endpoint, char *documents)
{
    ...
    websClose();
    LOGD("%s(%d)SUCCESS for %s\n", __FUNCTION__, __LINE__, documents);
    return 0;
}
